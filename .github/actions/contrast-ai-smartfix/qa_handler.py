...
# .github/contrast-ai-smartfix/qa_handler.py
import os
import sys
import shlex
import subprocess
...
def run_build_command(command: str, repo_root: Path) -> Tuple[bool, str]:
    ...
    try:
        ...
        return False, f"Error: Command not found: {command}"
...
def run_formatting_command(formatting_command: Optional[str], repo_root: Path) -> List[str]:
    ...
    if format_success:
        ...
        print(f"::error::Error executing formatting command: {formatting_command}", file=sys.stderr)
...
def run_qa_loop(
    ...
    formatting_command: Optional[str] # <<< ADDED
    ...
    while qa_attempts < max_qa_attempts:
        ...
        # Run QA agent
        qa_summary, requested_command = agent_handler.run_qa_agent(
        ...
        if requested_command:
            print(f"QA Agent requested command: `{requested_command}`")
            try:
                # Only run the requested command if a formatting_command was provided AND they match
                if formatting_command and requested_command == formatting_command:
                    print(f"Executing QA Agent requested formatting command: {formatting_command}")
                    # Use the run_formatting_command helper which already handles directory change
                    formatting_changed_files = run_formatting_command(formatting_command, repo_root)
                    # We'll consider it a success if it ran at all - actual changes may or may not have happened
                    format_success = True
                    if format_success:
                        debug_print("Formatting command successful.")
                        git_handler.stage_changes() # Stage any changes made by the formatter
                        if not git_handler.check_status(): # Check if formatter made changes
                            print("Formatting command ran but made no changes to commit.")
                            # Build should be re-run even if formatter made no git changes,
                            # as there might be in-memory changes or other side effects.
                        else:
                            debug_print("Formatting command made changes. Committing them.")
                            # <<< MODIFIED: Commit with a specific message for formatting >>>
                            git_handler.commit_changes(f"Apply formatting via: {formatting_command}")
                            # <<< END MODIFICATION >>>
                            changed_files = git_handler.get_last_commit_changed_files()
                    else:
                        print(f"Error executing formatting command: {formatting_command}", file=sys.stderr)
                        # No stdout/stderr available from run_formatting_command here, using a generic error
                        print(f"Formatting command failed", file=sys.stderr)
                        # Decide if we should break or continue if formatting fails. For now, continue to build.
                        qa_summary_log.append(f"Formatting command '{formatting_command}' failed.")
                elif formatting_command and requested_command != formatting_command:
                    print(f"\n::warning:: QA Agent requested command '{requested_command}', but it does not match the provided formatting command '{formatting_command}'. Ignoring agent's command request.")
                    qa_summary_log.append(f"QA Agent requested unapproved command: {requested_command}. Ignored.")
                elif not formatting_command and requested_command:
                    print(f"\n::warning:: QA Agent requested command '{requested_command}', but no FORMATTING_COMMAND was provided. Ignoring agent's command request.")
                    qa_summary_log.append(f"QA Agent requested command: {requested_command}, but no formatting command configured. Ignored.")
                ...
                # Re-run the main build command to check if the requested command fixed it
                print("\n--- Re-running Build Command After QA Command ---")
                build_success, build_output = run_build_command(build_command, repo_root)
                if build_success:
                    print("\n\u2705 Build successful after running QA requested command.")
                    break # Exit QA loop
                else:
                    print("\n\u274c Build still failing after running QA requested command.")
                    continue # Continue to next QA iteration
            except Exception as cmd_err:
                print(f"Exception running requested command '{requested_command}': {cmd_err}")
                continue # Continue to next QA iteration
        else:
            # No command requested, check if QA agent made file changes
