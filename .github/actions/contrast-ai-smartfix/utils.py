...
# .github/contrast-ai-smartfix/utils.py
import os
import subprocess
import sys
...
def ensure_gitignore_ignores_script_dir(repo_root: Path, script_dir_relative: str):
    ...
    try:
        if gitignore_path.exists():
            ...
            debug_print(f"{gitignore_path} does not exist. Creating it.")
            gitignore_path.touch()

        if not pattern_found:
            debug_print(f"Ignore pattern '{ignore_pattern}' not found. Adding it to {gitignore_path}.")
            ...
            debug_print("Ignore pattern added successfully.")
        else:
            debug_print(f"Ignore pattern '{ignore_pattern}' already exists in {gitignore_path}.")

    except Exception as e:
        print(f"Warning: Failed to check or update {gitignore_path}: {e}", file=sys.stderr)
        print("Proceeding without guaranteed .gitignore update.", file=sys.stderr)
