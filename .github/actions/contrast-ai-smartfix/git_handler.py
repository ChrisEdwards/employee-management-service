...
# .github/contrast-ai-smartfix/git_handler.py
import os
import sys
import datetime
...
def create_pr(title: str, body: str, head_branch: str, base_branch: str, label: str) -> str:
    ...
    try:
        ...
        # Allow script to continue if PR creation is optional, otherwise it would have exited via run_command
        return ""
    except Exception as e:
...
def get_pr_details_from_url(pr_url: str) -> dict:
    """Fetches and returns the details of a GitHub Pull Request given its URL."""
    debug_print(f"Fetching PR details from URL: {pr_url}")
    gh_env = os.environ.copy()
    gh_env["GITHUB_TOKEN"] = config.GITHUB_TOKEN
    ...
    try:
        ...
        if not match:
            print(f"Error: Could not parse GitHub URL: {pr_url}", file=sys.stderr)
            return {}
    ...
    # Now fetch the PR details using the GitHub CLI
    pr_number = pr_url.split("/")[-1] # Get the last part of the URL as the PR number
    pr_details_command = [
        "gh", "pr", "view", pr_number,
        "--repo", f"{owner}/{repo}",
        "--json", "number,title,body,author,createdAt,updatedAt,labels",
        "--jq", ". | {number, title, body, author: .author.login, createdAt, updatedAt, labels: [.labels[].name]} | tostring"
    ]

    try:
        pr_details_output = run_command(pr_details_command, env=gh_env, check=True)
        debug_print(f"PR details fetched: {pr_details_output}")
        return json.loads(pr_details_output)
    except json.JSONDecodeError:
        print(f"Warning: Could not parse JSON output from gh pr view: {pr_details_output}", file=sys.stderr)
        return {}
    except Exception as e:
